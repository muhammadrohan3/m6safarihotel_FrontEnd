...
function AddDrinkSales({ setAddOpen, salesData }) {
  ...
  const handleSubmit = () => {
    ...
    axios
      .post("/sales/addDrinkSales", {
        ...drink,
        total: Number(drink.total.toString().replace(",", "")),
  ...
  const handleDelete = () => {
    axios
      .delete(`/sales/deleteDrinkSales/${drink._id}`)
      .then((res) => {
        setMessage({ text: res.data.msg, type: "success" });
        setDrink({
          drinkItem: "",
  ...
  const handleUpdate = () => {
    ...
    axios
      .put(`/sales/updateDrinkSales/${drink._id}`, {
        ...drink,
        total: Number(drink.total?.toString().replace(",", "")),
        quantity: Number(drink.quantity?.toString().replace(",", "")),
      })
    ...
    >
      ...
      >
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setDrink({ ...drink, createdAt: e.target.value });
            }}
            value={drink?.createdAt}
          />
        </div>
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              setDrink({
                ...
                price: drinks?.find((item) => item._id === e.target.value)
                  ?.price,
                total:
                  Number(drink?.quantity?.toString().replace(",", "")) *
                  Number(
                    drinks
        ...
        <div>
            ...
            onChange={(e) => {
              if (isNumber1(e.target.value) || e.target.value === "") {
                if (
                  ...
                  drinks?.find((item) => item?._id === drink?.drinkItem)?.stock
                ) {
                  setDrink({
                    ...drink,
                    ...
                    total:
                      Number(e.target.value.toString().replace(",", "")) *
                      Number(drink?.price?.toString().replace(",", "")),
                  });
                } else {
                  setMessage({
                    text: "You cannot sell more than the available stock",
                    type: "error",
                  });
                }
              }
          ...
          {drink?.drinkItem && (
            <span className="flex w-full text-xs text-blue-600 justify-end">
              Available :{" "}
              {drinks?.find((item) => item?._id === drink?.drinkItem)?.stock}
            </span>
          )}
        ...
        <div>
          ...
          <input
            ...
            placeholder="Enter Amount here"
            onChange={(e) => {
              setDrink({ ...drink, stock: e.target.value });
            }}
            value={numberWithCommas(drink?.total)}
            disabled
          />
        </div>
        {drink?._id ? (
